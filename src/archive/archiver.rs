        ...
        if !self.allow_empty && self.archivable_stripe_count() == 0 {
            return Err(crate::ubiblk_error!(ArchiveError {
                description: "No stripes are marked as written or as existing in a stripe \
                              source, so the archive would contain no data. Either nothing \
                              has been written to the device yet, or it was written to with \
                              track_written disabled, which leaves the written flags unset. \
                              Pass --allow-empty to archive a device with no data."
                    .to_string(),
            }));
        }
