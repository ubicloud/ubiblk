...
are written in Python and drive the `archive`, `export-archive`, and
...
| `transient_errors_recovered_by_retries` | Errors that clear within the retry budget (fail some PutObjects, capped per object) do not fail a single archive. |
...
- `run_all.py` — launcher. Starts r2proxy in front of your bucket, points the
  tests at it, runs the cases, and deletes the run's objects on exit (pass,
  fail, or cancel).
- `cases.py` — the test cases and their harness (device fixture via
  `scripts/ubiblk-init`, archive/export drivers, r2proxy rule helpers).
...
Build the binaries once, then provide the **real** store's endpoint, bucket, and
keys and run the launcher; it takes care of the proxy, the config, and cleanup:

```sh
cargo build --bin archive --bin export-archive --bin init-metadata
...
- `cargo`, `python3`, and the `aws` CLI (used only for bucket cleanup).
